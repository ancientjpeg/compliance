<script lang="ts">
  ...
  const userOutput = $derived.by(async () =>
    transformToOutput(userInput, defaultReplacer),
  );
  ...
  const textBoxSharedStyle =
    "grow-1 basis-0 overflow-scroll resize-none p-4 text-neutral-400";
</script>
