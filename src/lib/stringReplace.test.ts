...
test("stringReplace replaces all words in default replacer", async () => {
  ...
  for (const key of defaultReplacer.keys()) {
    ...
    expect(await stringReplace(key, defaultReplacer)).toBe(
      defaultReplacer.get(key),
    );
    expect(await stringReplace(capitalize(key), defaultReplacer)).toBe(
      capitalize(defaultReplacer.get(key)!),
    );
    expect(await stringReplace(antiCapitalize(key), defaultReplacer)).toBe(
      defaultReplacer.get(key),
    );
    expect(await stringReplace(key.toUpperCase(), defaultReplacer)).toBe(
      defaultReplacer.get(key)!.toUpperCase(),
    );
  }
});
