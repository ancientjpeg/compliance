...
describe.each(testFiles)("Docx", (filePath) => {
  ...
  beforeEach(async () => {
    ...
    const suffix = ".docx";
    const pathBase =
      path.dirname(docPath) + "/" + path.basename(docPath, suffix);
  ...
  test("export operates as expected with stringReplace", async () => {
    ...
    if (!compDataExists) {
      ...
      expect.fail(
        "Writing new comparison data for repository, please verify and re-run tests.",
      );
    }

    expect(await getDocStrings(docPathOut)).toStrictEqual(
      await getDocStrings(docPathComp),
    );
  });
});
